<template>
  <div class="app-page">
    <PageHeader title="报告中心" subtitle="查看测试计划执行报告与明细">
      <template #actions>
    ...
    <el-card class="page-card" shadow="never">
      <el-table v-loading="listLoading" :data="list" border>
        ...
        <el-table-column label="计划" prop="plan_name" min-width="200" show-overflow-tooltip />
      ...
      <div class="mobile-cards">
        <div v-for="item in list" :key="item.id" class="mobile-card">
          ...
          <div class="mobile-card-meta">状态：{{ item.status }} · 成功：{{ item.pass_count }}/{{ item.total_count }}</div>
    ...
    <el-dialog v-model="detailVisible" title="报告详情" width="900px">
      <el-descriptions :column="3" border class="section-gap">
        ...
        <el-descriptions-item label="状态">{{ report.plan_run.status }}</el-descriptions-item>
        <el-descriptions-item label="摘要">{{ report.plan_run.summary }}</el-descriptions-item>
