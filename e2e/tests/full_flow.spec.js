...
test('生产级完整流程：项目-用例-计划-执行-报告', async ({ page }) => {
  ...
  await apiDialog.getByText('用例元信息', { exact: true }).click()
  const projectField = apiDialog.locator('.el-form-item').filter({ hasText: '所属项目' })
  await projectField.locator('.el-select').click()
  ...
  await apiDialog.getByRole('spinbutton', { name: '预期状态码' }).fill('200')
  ...
  const caseTypeField = caseDialog.locator('.el-form-item').filter({ hasText: '用例类型' })
  await caseTypeField.locator('.el-select').click()
  await page.getByRole('option', { name: 'API' }).click()
  await caseDialog.getByText('允许加入未通过评审用例', { exact: true }).click()
  const caseField = caseDialog.locator('.el-form-item').filter({ hasText: '选择用例' })
  await caseField.locator('.el-select').click()
