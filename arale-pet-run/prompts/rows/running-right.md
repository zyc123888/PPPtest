Create a single horizontal sprite strip for the Codex app digital pet `arale` in the state `running-right`.
...
Output exactly 8 separate animation frames arranged left-to-right in one single row. Each frame must show the same pet: Arale Norimaki inspired pet, simplified into Codex digital pet house style: compact chibi proportions, big round glasses, purple hair under a small winged cap, overalls, playful smile, tiny limbs, thick dark outline, limited palette, pixel-art-adjacent stepped edges, flat cel shading. Keep her iconic look readable but simplified. No scenery..
